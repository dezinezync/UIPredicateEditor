...
open class UIPredicateEditorViewController: UICollectionViewController {
  ...
  override open func collectionView(_: UICollectionView, contextMenuConfigurationForItemAt indexPath: IndexPath, point _: CGPoint) -> UIContextMenuConfiguration? {
    ...
    { [weak self] _ in
      ...
      predicateController.deleteRowTemplate(at: index)
      refreshContentView()
    ...
    return UIContextMenuConfiguration(identifier: nil, previewProvider: nil) { _ in
      UIMenu(
        ...
        children: additionalActions + [deleteAction].compactMap(\.self),
      )
    }
  }
}
...
extension UIPredicateEditorViewController {
  ...
  private func newRowMenuActions(for parentRowTemplate: UIPredicateEditorRowTemplate? = nil) -> [UIMenuElement] {
    let formattingHelper = formattingHelper

    var actions: [UIMenuElement] = rowTemplates.map { template in
    ...
    let comboAction = UIAction(
      title: NSLocalizedString("Combination", comment: ""))
    { [weak self] _ in
      #if DEBUG
      ...
      predicateController.addRowTemplate(template)
      collectionView.reloadData()
    }
