...
open class UIPredicateEditorCellContentView: UIView, UIContentView {
  ...
  private func performLayout(for width: CGFloat, apply: Bool) -> CGSize {
    ...
    var lines: [Line] = [Line()]
    let allViews: [UIView] = [leftExpressionView, operatorView, rightExpressionView].compactMap(\.self)
  ...
  weak var leftExpressionView: UIView?
  weak var operatorView: UIView?
  weak var rightExpressionView: UIView?
  weak var trailingButton: UIButton?

  var appliedConfiguration: UIPredicateEditorCellConfiguration!

  func apply(configuration: UIPredicateEditorCellConfiguration) {
...
extension UIPredicateEditorCellContentView: UIPredicateEditorContentRefreshing {
  public func refreshContentView() {
    ...
    appliedConfiguration.delegate?.refreshContentView()
  }
}
#endif
